...
<!-- Breadcrumb End -->
  ...
  <div class="card border-0 shadow" style="border-radius: 16px; margin-top: 20px; background: #f1f5ff;">
    <div class="card-body p-5" style="min-height: 80vh; display: flex; align-items: center; justify-content: center; flex-direction: column; font-family: 'Segoe UI', sans-serif;">

      <div class="text-center" style="max-width: 720px; width: 100%;">
        <h2 style="font-weight: 700; font-size: 2rem; color: #1e3a8a;">🌾 Smart Crop Recommendation</h2>
        <p style="color: #374151; font-size: 1.1rem; margin-bottom: 24px;">
          This tool detects your location, fetches the weather forecast, and recommends the top crops to grow.
        </p>
        <button onclick="getCropRecommendation()" style="
          background: #2563eb;
          color: white;
          padding: 10px 20px;
          font-size: 1rem;
        ...
        <h3 style="color: #1f2937; font-weight: 600; font-size: 1.25rem;">🧠 AI Suggestions:</h3>
        <pre id="result" style="
          background: #ffffff;
          padding: 16px;
          border-radius: 10px;
...
<script>
  const WEATHER_API_KEY = "aKE1aioDJfUqzS1zzttwYFDqUHdh7ep1";
  const GEMINI_API_KEY = "AIzaSyDx_ByAyeTR1PIW7s4fZrqs_lb7yAP1Ts4";

  async function getCropRecommendation() {
    const resultBox = document.getElementById("result");
    resultBox.textContent = "⏳ Getting your location...";

    navigator.geolocation.getCurrentPosition(async (pos) => {
      const lat = pos.coords.latitude;
      const lon = pos.coords.longitude;
      resultBox.textContent = `📍 Location: ${lat}, ${lon}\nFetching weather forecast...`;

      try {
        const weatherUrl = `https://api.tomorrow.io/v4/weather/forecast?location=${lat},${lon}&apikey=${WEATHER_API_KEY}`;
        const weatherResponse = await fetch(weatherUrl);
        const weatherData = await weatherResponse.json();

        resultBox.textContent = "📦 Weather data retrieved. Asking RIG LAB for best crops...";

        const prompt = `
You are an expert agricultural AI.
Given the following 5-day weather forecast, suggest the 5 to 6 most suitable crops to grow in that region.
Explain briefly why each crop is a good fit based on the forecast.in your answer do not tell 5-day

Weather data (do not display to user):
${JSON.stringify(weatherData)}

Only include crop name and a short reason for each. Use **bold** for crop names.
`;
        ...
        let output = result?.candidates?.[0]?.content?.parts?.[0]?.text || "❌ RIG LAB did not return a response.";

        // Convert **bold** from markdown to <strong>
      ...
      resultBox.textContent = "❌ Location error: " + err.message;
    });
  }
</script>
