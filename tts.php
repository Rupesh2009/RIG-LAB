...
<div class="container-fluid">
  <div class="card border border-gray-100 w-100" style="border-radius: 16px; overflow: hidden; margin-top: 20px;">
    <div class="card-body p-4 d-flex flex-column align-items-center justify-content-center" style="background-color: #eaf0ff; min-height: 90vh;">
      
      <h3 class="mb-3"><i class="bi bi-journal-richtext"></i> Story Generator</h3><br>

      <input type="text" id="classInput" class="form-control mb-3" placeholder="Enter Class (e.g., Class 6)" style="max-width: 600px; background-color: #fff;">
      <input type="text" id="chapterInput" class="form-control mb-3" placeholder="Enter Chapter Name (e.g., Water Cycle)" style="max-width: 600px; background-color: #fff;"><br>

      <button onclick="generateStory()" class="btn btn-info mb-4 fw-bold"><i class="bi bi-stars"></i> Generate Story</button><br>

      <h5 class="mb-2"><i class="bi bi-book-half"></i> Story:</h5>
      <textarea id="storyOutput" class="form-control mb-3" rows="10" readonly style="max-width: 800px; background-color: #fff;"></textarea><br>

      <div class="d-flex gap-3">
        <button onclick="speakStory()" class="btn btn-success"><i class="bi bi-play-fill"></i> Play</button>
        <button onclick="stopSpeaking()" class="btn btn-danger"><i class="bi bi-stop-fill"></i> Stop</button>
      </div>
...
<script>
const GEMINI_API_KEY = "AIzaSyDx_ByAyeTR1PIW7s4fZrqs_lb7yAP1Ts4";

function cleanText(text) {
  return text
    .replace(/\*+/g, '')
    .replace(/\(Pause\)/gi, '')
    .replace(/\n{2,}/g, '\n\n') // double line breaks for paragraph feel
    .trim();
}
...
async function generateStory() {
  ...
  if (!classInput || !chapterInput) {
    alert("Please enter both class and chapter name.");
    return;
  }

  storyOutput.value = "⏳ Generating story...";
  ...
  try {
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${GEMINI_API_KEY}`, {
      ...
      body: JSON.stringify({
        contents: [{
          parts: [{
            text: `Create a well-structured, paragraph-based Hinglish story for Class ${classInput} to explain the concept of "${chapterInput}". Use a mix of Hindi and English, fun narration, and smooth transitions. Do not include the word "pause", asterisks, or any formatting markers. Just clear and engaging storytelling as if reading it aloud.`
          }]
        }]
      })
    });
    ...
    const storyRaw = data.candidates?.[0]?.content?.parts?.[0]?.text || "❌ No story generated.";
    storyOutput.value = cleanText(storyRaw);

  } catch (error) {
    storyOutput.value = "❌ Error generating story.";
    console.error("Error:", error);
  }
}

function speakStory() {
  const story = cleanText(document.getElementById("storyOutput").value);
  if (!story) return;

  responsiveVoice.speak(story, "Hindi Female", {
    rate: 0.95,
    pitch: 1.1
  });
}
...
<!-- ResponsiveVoice JS -->
<script src="https://code.responsivevoice.org/responsivevoice.js?key=Tt89qT6j"></script>
