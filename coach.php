...
<!-- Breadcrumb End -->
  ...
  <div class="card border border-gray-100 w-100" style="border-radius: 16px; overflow: hidden; margin-top: 20px;">
    <div class="card-body p-0" style="background-color: #eaf0ff; height: 80vh; overflow-y: auto;">
      <div class="sc-dashboard">
        <div class="sc-header">
          <h1>🧠 AI Study Coach</h1>
          <p>Your personal mentor for daily study success</p>
        </div>

        <div class="sc-container">
          <label for="userInput">📝 What did you study today? How are you feeling?</label>
          <textarea id="userInput" rows="4" placeholder="E.g. Studied trigonometry. Feeling okay but distracted."></textarea>

          <label for="energyLevel">⚡ Your Energy Level</label>
          ...
          <label for="focusLevel">🎯 Focus Level (1–5)</label>
          <select id="focusLevel">
            <option value="5">5 - Super focused</option>
          ...
          <button onclick="getCoachResponse()">✨ Get My Advice</button>
          <div id="response" class="sc-card" style="display: none;"></div>
        </div>
      </div>
    </div>
  </div>
...
<script>
  const API_KEY = "AIzaSyDx_ByAyeTR1PIW7s4fZrqs_lb7yAP1Ts4"; // Replace with your Gemini API key
  ...
  async function getCoachResponse() {
    ...
    if (!input) return alert("Please describe what you studied and how you're feeling.");

    const prompt = `
Tum ek friendly aur motivating AI study coach ho. Student ne bola: "${input}".
Uska energy level hai: ${energy}, aur focus level hai: ${focus}/5.
Ab tum usey ek daily guidance do:

1. Ek chhoti si motivational line Hinglish mein
2. Aaj ke liye ek simple study task
3. Ek focus badhane ki technique
4. Kal ke liye ek chhoti study habit

Bas Hinglish mein reply karo. English translation ya bracket waale sentences bilkul mat dena.
Sirf casual, asaan aur dosti bhara tone ho.
    `;
    ...
    try {
      ...
      const uiText = raw.trim();
      const speechText = uiText
        .replace(/[\*\[\]\(\)#\d\.]/g, '') // added \. to remove dots
        .replace(/\s{2,}/g, ' ')

        .trim();

      resBox.innerHTML = uiText;
      responsiveVoice.speak(speechText, "Hindi Female", { rate: 1.0 });
    } catch (err) {
      resBox.innerHTML = "❌ Error getting response.";
    }
  }
</script>
...
<style>
  ...
  margin: 0;
  font-size: 24px;
  color: #ffffff; /* <-- make heading text white */
}
  ...
  .sc-container textarea:focus,
  .sc-container select:focus {
    outline: none;
    border-color: #3b82f6;
